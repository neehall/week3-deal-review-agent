...
No external dataset — this project's "data" is a hand-built, 4-tier synthetic
test set, since the point was to control exactly which compliance/risk
issues are present and verify the pipeline catches them:
- **`data/compliance/rules.yaml`** — 6 synthetic compliance rules (rate cap,
  required effective date, termination-clause requirement, minimum party
  count, arbitration-disclosure requirement, required deal amount), scoped
  for a lending/commercial-deal review context.
- **`data/sample_deals/`** — 20 synthetic deal documents (loan agreements,
  leases, term sheets, vendor/services/financing agreements) across 4
  tiers: `normal/` (clean baselines), `failing/` (unambiguous violations),
  `edge_cases/` (boundary/ambiguous cases — exact-threshold values,
  conflicting dates, foreign currency formatting), and `extreme/`
  (pipeline-mechanics stress tests — empty files, gibberish input, a
  document containing an embedded prompt-injection attempt, and a long
  document that surfaced a real `max_tokens` truncation bug).
- Every document's expected/observed behavior is recorded in
  `data/sample_deals/ANSWER_KEY.md`, generated by actually running the live
  graph against each file, not predicted by hand.
...
Per the handout's "How to submit" section — three deliverables, submitted
via the Google Form (https://forms.gle/HMgTU7zy6UJ8XkJX6):

- [x] Framework primer + table (`FRAMEWORK.md`) — prerequisite, not itself
      a submitted deliverable, but required before building.
- [x] **Code base**: working code (LangGraph + LangChain, Track 2), pushed
      to a public GitHub repo: https://github.com/neehall/week3-deal-review-agent
- [x] Architecture diagrams (`docs/ARCHITECTURE.md`) — supporting doc, not
      separately required, but strengthens the write-up.
- [x] **Project documentation** content drafted (this file: overview,
      datasets used, prompts, iterations, learnings) — **still needs to be
      copy-pasted into an actual Google Doc** and that Doc's share link
      captured for the form; a markdown file in the repo does not by itself
      satisfy "submit a Google Doc."
- [ ] **Video demo** (≤5 min: walk through the app, explain what was built,
      describe how AI coding tools were used, demonstrate the result live)
      — not yet recorded.
- [ ] **Form submission** — submit the Google Doc link, video link, and
      GitHub link via the submission form before the Aug 30, 2026 deadline.
      Nothing has been submitted through the form yet as of this write-up.
